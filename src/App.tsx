...
const AppContent: React.FC = () => {
  ...
  // Map center: use user's GPS location, or fall back to Vancouver (Canadian pilot city)
  // Cairo ([30.0444, 31.2357]) removed — location is always GPS-first
  const center: [number, number] = userLocation ?? [49.2827, -123.1207];
          ...
          try {
            ...
            if (!Capacitor.isNativePlatform()) {
              try {
                ...
                const defaultLocation: [number, number] = [49.2827, -123.1207]; // Vancouver, BC (pilot city)
                console.log('✅ Setting default location:', defaultLocation);
                setUserLocation(defaultLocation);
                console.log(`📍 Using default location: Vancouver (${defaultLocation[0]}, ${defaultLocation[1]}) - marker should appear on map`);
              }
            } else {
              ...
              try {
                ...
                if (data.latitude && data.longitude) {
                  ...
                  setUserLocation(defaultLocation);
                  console.log(`📍 Mobile using default location: Vancouver (${defaultLocation[0]}, ${defaultLocation[1]}) - marker should appear`);
                }
              } catch (mobileError) {
                console.log('❌ Mobile IP fallback failed:', (mobileError as Error)?.message || String(mobileError));
  ...
  const handleLanguageSelect = async (language: 'en' | 'ar' | 'fr') => {
    ...
    // Set document direction and language — only Arabic is RTL, French is LTR
